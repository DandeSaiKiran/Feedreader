...
class MainActivity : AppCompatActivity(), FeedAdapter.FeedItemListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        feedAdapter = FeedAdapter(arrayList, this)
    ...
    override fun onLikeClicked(model: FeedReaderModel) {
        Log.d(TAG, "onLikeClicked  FeedReaderModel: like =  ${model.likes}")
        val feedApi: FeedApi = FeedClient().getClient().create(FeedApi::class.java)


        val modelFeed = FeedReaderModel(id = model.id,likes = model.likes,
                title = model.title,
                author = model.author, description = model.description,image = "")

        val callLike: Call<FeedReaderModel> = feedApi.postApiDetails(feedReaderModel = modelFeed)

        callLike.enqueue(object :  Callback<FeedReaderModel> {
            override fun onResponse(call: Call<FeedReaderModel>?, response: Response<FeedReaderModel>?) {
                if (response!!.isSuccessful){

                    Log.d(TAG,"onLikeClicked onResponse like = ${modelFeed.likes}")
                }
            }
